...
function SerieSingleCard({ tv, indx }) {
    const ranking = Math.round(tv.vote_average / 2);

    return (
        <div>
            <Link href={`/tv/${tv.id}`} key={indx}>
                <Box
                    ...
                    borderRadius={10}
                    backgroundImage={`https://image.tmdb.org/t/p/w500${tv.poster_path}`}
                >
                    <span>
                        <Flex ml={"10%"} pb="20px">
                            {_.times(ranking).map(() => (
...
export default SerieSingleCard;
